...
export function isLongWorkoutDuration(durationSeconds, thresholdSeconds = LONG_WORKOUT_SECONDS) {
  const duration = finiteNumber(durationSeconds, 0);
  const threshold = Math.max(1, finiteNumber(thresholdSeconds, LONG_WORKOUT_SECONDS));
  return duration > threshold;
}

export function isStaleWorkoutDraft(draft, now = Date.now(), thresholdSeconds = LONG_WORKOUT_SECONDS) {
  const normalized = normalizeWorkoutDraft(draft);
  return normalized ? isLongWorkoutDuration(elapsedSecondsSince(normalized.sessionStartTime, now), thresholdSeconds) : false;
}

export function resetWorkoutDraftSessionTimer(draft, now = Date.now()) {
  const normalized = normalizeWorkoutDraft(draft);
  if (!normalized) return null;
  const resetAt = finiteNumber(now, Date.now());
  return {
    ...normalized,
    sessionStartTime: resetAt,
    savedAt: resetAt,
  };
}

export function actualDurationSecondsFromMinutes(value) {
